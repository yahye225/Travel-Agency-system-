...
CREATE DEFINER=`root`@`localhost` PROCEDURE `rp_deleted_sp` (`_table` TEXT, `_from` DATE, `_to` DATE, `_user_id` INT)   BEGIN

SELECT 'undo_sp' `sp~req hide`,`backup` `Back Up~req`,table_ `Table~req`,d.cols `col~req hide`,u.username,d.date,`undo`()`Undo` FROM deleted_records d JOIN users u on d.user_id=u.id
WHERE date(d.date) BETWEEN _from AND _to AND d.user_id LIKE _user_id AND d.table_ LIKE _table;
END$$
...
INSERT INTO `deleted_records` (`id`, `tran_id`, `table_`, `backup`, `cols`, `user_id`, `date`, `description`) VALUES
(1, 9, 'branch', '9,bakaaro,2147483647,Tanzania,2023-04-30,admin,767', 'id,name,tell,address,date,admin,admintell', 2, '2023-04-30 00:00:00', NULL);
